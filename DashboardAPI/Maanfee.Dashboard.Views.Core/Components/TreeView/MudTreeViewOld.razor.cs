...
#nullable enable
    public partial class MudTreeViewOld<T> : MudComponentBase
    {
        ...
        private HashSet<MudTreeViewItemOld<T>>? _selectedValues;
        private List<MudTreeViewItemOld<T>> _childItems = new();
        ...
        [CascadingParameter]
        private MudTreeViewOld<T> MudTreeRoot { get; set; }

        [Parameter]
        [Category(CategoryTypes.TreeView.Data)]
        public Func<T?, Task<HashSet<T>>>? ServerData { get; set; }

        public MudTreeViewOld()
        {
            MudTreeRoot = this;
        }
        ...
        internal Task SetSelectedItemsCompare()
        {
            ...
            {
                _selectedValues ??= new(new MudTreeViewItemComparerOld<T>(Comparer));
        ...
        public async Task Select(MudTreeViewItemOld<T> item, bool isSelected = true)
        {
            if (MultiSelection)
            {
        ...
        internal void AddChild(MudTreeViewItemOld<T> item) => _childItems.Add(item);
        ...
        {
            List<MudTreeViewItemOld<T>> unselectedItem;
            ...
            lock (_selectedUpdateLock)
            {
                ...
                var newSelected = new HashSet<MudTreeViewItemOld<T>>(new MudTreeViewItemComparerOld<T>(comparer));
        ...
        internal async Task UnSelectAllChildren(List<MudTreeViewItemOld<T>>? children = null)
        {
        ...
        internal MudTreeViewItemOld<T>? FindItemByValue(T value, List<MudTreeViewItemOld<T>>? children = null)
        {
            children ??= _childItems;
