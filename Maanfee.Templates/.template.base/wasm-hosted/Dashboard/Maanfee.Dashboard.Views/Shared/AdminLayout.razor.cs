...
namespace Maanfee.Dashboard.Views.Shared
{
    public partial class AdminLayout : SharedLayout
    {
        ...
        {
            if (ConfigurationModel.IsDarkMode)
            {
                ConfigurationModel.IsDarkMode = false;
            }
            else
            {
                ConfigurationModel.IsDarkMode = true;
            }

            CurrentTheme = ConfigurationModel.IsDarkMode ? MaanfeeTheme.DarkTheme : MaanfeeTheme.DefaultTheme;

            await LocalStorage.SetAsync<ConfigurationModel>(StorageDefaultValue.ConfigurationStorage, ConfigurationModel);
        }
