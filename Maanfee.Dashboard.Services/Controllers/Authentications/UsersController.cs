	...
	public class UsersController : _BaseController
	{
		...
		public async Task<CallbackResult<IEnumerable<ApplicationUser>>> GetUserByIdDepartment(int IdDepartment, string Value)
		{
			try
			{
				...
				{
					Users = Users.Where(x => x.Name == Value).ToList();
				}
		...
		public async Task<CallbackResult<PaginatedList<GetUserViewModel>>> PaginationIndex(TableStateViewModel<string> TableState)
		{
			try
			{
				...
				if (!string.IsNullOrEmpty(TableState.Filter))
				{
					Data = Data.Where(
						p => p.Name.Contains(TableState.Filter) ||
						p.UserName.Contains(TableState.Filter) ||
						p.PersonalCode.Contains(TableState.Filter));
		...
		public async Task<CallbackResult<IList<DropDownDepartmentViewModel>>> GetAvailableDepartmentByIdDepartment(int? IdDepartment)
		{
			try
			{
